          ...
          getPropertyValue: function(e, r, n, o) {
            function s(e, r) {
              ...
              if (8 >= d) l = f.css(e, r);
                ...
                let c = e.offsetHeight - (parseFloat(S.getPropertyValue(e, "borderTopWidth")) || 0) - (parseFloat(S.getPropertyValue(e, "borderBottomWidth")) || 0) - (parseFloat(S.getPropertyValue(e, "paddingTop")) || 0) - (parseFloat(S.getPropertyValue(e, "paddingBottom")) || 0);return n(), c;
                  }
                  if (
                    'width' === r &&
                    'border-box' !==
...
!(function(a, b, c, d) {
  ...
  function Gb() {
    ab.apply(this, arguments);let a = q(this.handler, this);this.touch = new Eb(this.manager, a), this.mouse = new rb(this.manager, a);
  }
  function Pb(a, b) {
  ...
  (Yb.prototype = {
    ...
    p(cc, ac, {
      ...
      emit: function(a) {
        ...
        let b = a.scale < 1 ? "in" : "out";this.manager.emit(this.options.event + b, a);
        }
      },
    }),
    p(dc, Yb, {
    ...
    p(fc, ac, {
      ...
      emit: function(a) {
      let b = $b(a.direction);b && this.manager.emit(this.options.event + b, a), this.manager.emit(this.options.event, a);
      },
    }),
    p(gc, Yb, {
      defaults: {
  ...
  (kc.prototype = {
    ...
    emit: function(a, b) {
      this.options.domEvents && mc(a, b);let c = this.handlers[a] && this.handlers[a].slice();if (c && c.length) {
        (b.type = a),
...
(function($) {
  ...
  $.fn.dropdown = function(options) {
    ...
    this.each(function() {
      let origin = $(this);
      let curr_options = $.extend({}, defaults, options);
      let isFocused = false;
      ...
      */
      function placeDropdown(eventType) {
        ...
        let originWidth = origin[0].getBoundingClientRect().width;
...
(function($) {
  $.fn.materialbox = function() {
    return this.each(function() {
      ...
      let outDuration = 200;
      let origin = $(this);
      let placeholder = $('<div></div>').addClass('material-placeholder');
      let originalWidth = 0;
      let originalHeight = 0;
      let ancestorsChanged;
      let ancestor;
      let originInlineStyles = origin.attr('style');
      ...
      function returnToOriginal() {
        ...
        let placeholder = origin.parent('.material-placeholder');
        let windowWidth = window.innerWidth;
        let windowHeight = window.innerHeight;
        let originalWidth = origin.data('width');
        let originalHeight = origin.data('height');
...
(function($) {
  $.fn.tooltip = function(options) {
    ...
    return this.each(function() {
      let tooltipId = Materialize.guid();
      let origin = $(this);
      ...
      let renderTooltipEl = function() {
        let tooltip = $('<div class="material-tooltip"></div>');
      ...
      origin.on({
        'mouseenter.tooltip': function(e) {
          let showTooltip = function() {
            ...
            let tooltipHeight = tooltipEl.outerHeight();
            let tooltipWidth = tooltipEl.outerWidth();
            let tooltipVerticalMovement = '0px';
            let tooltipHorizontalMovement = '0px';
            let backdropOffsetWidth = backdrop[0].offsetWidth;
            let backdropOffsetHeight = backdrop[0].offsetHeight;
            let scaleXFactor = 8;
            let scaleYFactor = 8;
            let scaleFactor = 0;
...
(function($) {
  ...
  function onScroll(scrollOffset) {
    ...
    let intersections = findElements(
  ...
  $.scrollSpy = function(selector, options) {
    ...
    let throttledScroll = Materialize.throttle(() => {
      onScroll(options.scrollOffset);
    }, options.throttle || 100);
    let readyScroll = function() {
...
(function($) {
  ...
  $.fn.material_select = function(callback) {
    $(this).each(function() {
      ...
      let appendOptionWithIcon = function(select, option, type) {
        // Add disabled attr if disabled
        let disabledClass = option.is(':disabled') ? 'disabled ' : '';
        let optgroupClass =
          type === 'optgroup-option' ? 'optgroup-option ' : '';
        let multipleCheckbox = multiple
          ? '<input type="checkbox"' + disabledClass + '/><label></label>'
          : '';

        // add icons
        let icon_url = option.data('icon');
      ...
      if (selectChildren.length) {
        selectChildren.each(function() {
          if ($(this).is('option')) {
            ...
            // Optgroup.
            let selectOptions = $(this).children('option');
            options.append(
              $(
      ...
      let dropdownIcon = $('<span class="caret">&#9660;</span>');

      // escape double quotes
      let sanitizedLabelHtml = label.replace(/"/g, '&quot;');
...
(function($) {
  ...
  $.fn.material_chip = function(options) {
    ...
    let curr_options = $.extend({}, materialChipsDefaults, options);
    ...
    this.handleEvents = function() {
      ...
      self.$document
        ...
        let $currChips = $(e.target).closest(SELS.CHIPS);
    ...
    this.renderChip = function(elem) {
      ...
      let $renderedChip = $('<div class="chip"></div>');
    ...
    this.addChip = function(elem, $chips) {
      ...
      let $renderedChip = self.renderChip(elem);
      let newData = [];
      let oldData = $chips.data('chips');
    ...
    this.deleteChip = function(chipIndex, $chips) {
      let chip = $chips.data('chips')[chipIndex];
      $chips
        .find('.chip')
        .eq(chipIndex)
        .remove();
    ...
    this.selectChip = function(chipIndex, $chips) {
      let $chip = $chips.find('.chip').eq(chipIndex);
...
(function($) {
  ...
  var FABtoToolbar = function(btn) {
    ...
    let anchor = btn.find('> a').first();
    let menu = btn.find('> ul').first();
  ...
  var toolbarToFAB = function(btn) {
    ...
    let windowWidth = window.innerWidth;
    let windowHeight = window.innerHeight;
    let btnWidth = btn.attr('data-origin-width');
    let btnBottom = btn.attr('data-origin-bottom');
    let btnLeft = btn.attr('data-origin-left');
    let anchor = btn.find('> .btn-floating').first();
    let menu = btn.find('> ul').first();
    let backdrop = btn.find('.fab-backdrop');
    let fabColor = anchor.css('background-color');
...
(function($) {
  ...
  let transitionSupported = (function() {
  ...
  function ClockPicker(element, options) {
    ...
    if (svgSupported) {
      ...
      let bearing = createSvgElement('circle');
      bearing.setAttribute('class', 'clockpicker-canvas-bearing');
      bearing.setAttribute('cx', 0);
      bearing.setAttribute('cy', 0);
      bearing.setAttribute('r', 4);
      let hand = createSvgElement('line');
      hand.setAttribute('x1', 0);
      hand.setAttribute('y1', 0);
      let bg = createSvgElement('circle');
